...
export default function Chat({ messages: initialMessages }: { messages: Message[] }) {
  ...
  const renderToolInvocation = (toolInvocation: any) => {
    ...
    switch (toolInvocation.toolName) {
            ...
            <div className="pt-[0.5rem]">
              {result && (
                <div className='flex gap-1'>
                  <Link href={`https://${result.site_subdomain}.tsafi.xyz`} target="_blank">
      ...
      case 'read_sites':
        return (
          <div key={toolCallId} className='flex gap-2 flex-col bg-blue-700 bg-opacity-10 text-sm whitespace-pre-wrap px-3 py-2 rounded-lg'>
            <p>{message}</p>
          ...
          {messages.map((m: any) => (
            <div key={m.id} className={m.role === "user" ? "flex flex-col justify-center items-end w-full gap-1 mb-4" : "flex flex-col justify-center items-start w-full gap-1 mb-4"}>
              <div className="flex flex-col gap-1 max-w-[80%]">
                ...
                {m?.content && (
                  <div className='bg-blue-700 whitespace-pre-wrap bg-opacity-10 text-sm px-3 py-2 rounded-lg'>
                    {m?.content}
                    <div className='flex gap-2 flex-wrap w-full'>
                      {m?.type === "tool-result_read-site" && m?.result?.map((info: any) => (
                        <Link key={info.id} href={`/cms/sites/${info.site_subdomain}`} prefetch={true} className="flex flex-col rounded-md w-[350px] hover:cursor-pointer transition-shadow duration-300" target='_blank'>
                          <Card className="flex flex-col px-[1rem] justify-between h-full py-[1rem]">
                            <div className='flex flex-col w-full justify-center items-start'>
