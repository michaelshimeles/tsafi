import BlogNavBar from "@/components/BlogBuilder/BlogNavBar";
...
export default async function BlogPage({ params }: { params: { id: string } }) {
  const response = await getAllArticlesApi(params?.id)
  return (
    <main className="flex min-w-screen flex-col items-center justify-between ">
      ...
      <div className="flex flex-col mt-[1rem] justify-center items-center w-[90%]">
        ...
        <div className="grid gap-5 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 mt-5">
          {response?.map((info: Article) => (
            <Link key={info?.id} href={`/blog/${params?.id}/${info?.slug}`}>
              <article
